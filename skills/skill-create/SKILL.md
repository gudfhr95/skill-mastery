---
name: skill-create
description: Use when the user wants to design, create, scaffold, review, or improve an AI agent skill, including repo-local skills, Codex skills, skill templates, trigger descriptions, bundled resources, agents/openai.yaml metadata, validation plans, and proposal-first skill improvement. Do not use for ordinary git commits, pull request creation, or unrelated repository documentation work.
---
...
3. Design the skill boundary: name, description trigger, non-goals, workflow,
   resources, invocation policy, validation checks, success criteria, and
   improvement loop.
...
6. Add `scripts/`, `references/`, `assets/`, or additional `templates/` only
   when they directly improve repeatability or output quality.
7. Validate the result with checks that match the change: frontmatter shape,
   naming rules, stale template markers, metadata consistency, resource
   relevance, and any available repo validation command.
8. Report the created or changed files, validation performed, skipped checks,
